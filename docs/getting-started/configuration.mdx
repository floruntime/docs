...
[cluster]
enabled = false
# secret = "..."          # required whenever the Raft port is bound; same value on every node
# node_id = 1
...
crosses the wire. Generate one with `openssl rand -base64 32`. Starting the
Raft listener without it is a startup error that names the key.
