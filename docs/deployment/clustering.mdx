...
:::note
**Alarm on the `flo_replication_*` counters.** Cross-node entry replication is currently best-effort broadcast: there is no acknowledgement, send-side gap detection, or repair path. If a follower misses a committed entry it stays missing — the leader has already acked the client.

These counters exist so that failure is **visible** rather than silent. Any non-zero value means a replica has diverged and needs to be rebuilt from the leader. Divergence detection is implemented; automatic repair is not — see [issue #16](https://github.com/floruntime/flo/issues/16).
:::
