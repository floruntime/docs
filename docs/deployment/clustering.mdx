...
The node exposes replication counters as JSON on the dashboard's metrics
endpoint, under `replication`:

```bash
curl localhost:9002/api/v1/metrics
```

| Metric | Description |
|--------|-------------|
| `follower_gaps_total` | Replication gaps detected on a follower |
| `follower_entries_missing_total` | Entries inferred missing across all gaps |
| `broadcast_oversize_skipped_total` | Committed entries too large to broadcast |
| `broadcast_send_failures_total` | Per-peer sends that failed and were not retried |
| `last_gap_received_index` | Entry index at which a gap was last observed |

:::note
**Alarm on these counters.** Cross-node entry replication is currently
best-effort broadcast: there is no acknowledgement, send-side gap detection, or
repair path. If a follower misses a committed entry it stays missing — the
leader has already acked the client.
...
:::note
**A Prometheus text endpoint is not served yet.** The exporter exists in the
codebase but nothing starts it at runtime, so there is no `/metrics` scrape
target on port 9001 today. Cluster-level metric names such as
`flo_cluster_nodes`, `flo_raft_leader_elections_total` and `flo_gossip_members`
are not implemented either. Use the dashboard endpoint above until the exporter
is wired up.
:::
