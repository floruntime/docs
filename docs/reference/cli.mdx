---
title: CLI Reference
lede: Complete reference for the flo command-line interface.
---
...
| Flag | Description |
|------|-------------|
| `-e, --endpoint <host:port>` | Server endpoint (default: `localhost:9000`) |
| `-p, --port <port>` | Server port, when the host is the default |
| `-n, --namespace <name>` | Namespace for operations (default: `default`) |
| `-o, --output <fmt>` | Output format: `table`, `json`, `raw` |
...
| `--data-dir <path>` | Data directory | `~/.flo/data` |
| `--shards <n>` | Number of shard threads, at most 256; a cluster member runs 1 (the default resolves to it) | CPU count |
| `--cluster` | Start as the first member of a cluster; leads alone until others join | — |
| `--join <host:port,...>` | Join members that exist, by their peer port (`listen_port + 500`) | — |
| `--node-id <id>` | Unique node identifier | derived from `hostname:port` |
| `--raft-port <port>` | Peer port, with `--cluster` or `--join` | `listen_port + 500` |
...
| Flag | Description |
|------|-------------|
| `--wait <ms>` | Wait until the key exists (at most 300000 ms, 5 minutes; 0 does not wait; a longer wait is refused) |
| `--block <ms>` | Wait for the key's next change (at most 300000 ms, 5 minutes; 0 does not wait; a longer wait is refused). On timeout, prints the unchanged value, or not found if the key is gone |
...
```bash
TXN=$(flo kv begin user:42 -o json | jq -r .txn_id)
...
| `--block <ms>` | Wait for new records (at most 300000 ms, 5 minutes; 0 does not wait; a longer wait is refused) |
...
| `--block <ms>` | Wait for messages (at most 300000 ms, 5 minutes; 0 does not wait; a longer wait is refused) |
...
| Flag | Description |
|------|-------------|
| `--run-id <id>` | Choose the run's id. Ids of the form `wfr-<hex>-<hex>` are the server's and are refused |
...
Show this node's id, role (`leader`, `follower`, `electing`, `joining`,
`diverged`), the leader it knows (`none` while electing or joining), the term
and the member count. `-o json` gives the same as one object.
...
CLI but not yet implemented by the server.
