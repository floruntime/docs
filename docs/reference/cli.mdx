...
| Flag | Description |
|------|-------------|
| `--wait <ms>` | Wait until the key exists (at most 5 minutes; 0 or anything longer means 5 minutes) |
| `--block <ms>` | Wait for the key's next change (at most 5 minutes; 0 or anything longer means 5 minutes). On timeout, prints the unchanged value, or not found if the key is gone |
...
| `--block <ms>` | Wait for new records (at most 5 minutes; 0 or anything longer means 5 minutes) |
...
| `--block <ms>` | Wait for messages (at most 5 minutes; 0 or anything longer means 5 minutes) |
