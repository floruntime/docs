---
title: Web Console
lede: The built-in web dashboard — a read-and-act UI for every Flo primitive, served on port 9002.
---
...
| **Time Series** | Namespace-scoped measurements, per-field point charts pulled from the raw write buffers, a FloQL console that executes queries server-side and renders the resulting series, and a line-protocol ingest helper. |
...
The dashboard thread has **read-only** access to the projections, so read endpoints serve data directly. Mutations (KV writes, queue enqueue/purge/requeue, stream trim/delete, action invoke, processing submit/stop/cancel, workflow create/start/signal/cancel/enable/disable) are proposed through the node's normal write path, so they are durably replicated just like a CLI or SDK call.

Not every endpoint has a control in the UI yet — stream trim/delete, queue purge, and workflow create/signal are reachable over the [REST API](/reference/rest-api/) but are not surfaced as buttons. Some operations remain CLI-only and appear in the console as a copyable command, such as registering an action. See the [CLI reference](/reference/cli/).
