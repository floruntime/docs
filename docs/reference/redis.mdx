...
lede: Use redis-cli, ioredis, go-redis, redis-py and other Redis clients against Flo.
---
...
| `BRPOP queue timeout` | — | Not supported. Use `flo queue dequeue --block` or an SDK for a blocking dequeue. |
...
| `XREAD COUNT n STREAMS stream id` | `stream_read` | One stream. `BLOCK` is accepted but ignored: the read returns at once. |
...
| Area | Behavior |
|------|----------|
| Persistence | Stream appends (`XADD`) and queue pushes (`LPUSH`/`RPUSH`) go through the log and are answered once they apply. Key-value writes over RESP (`SET`, `DEL`, `INCR`, …) go to the node's memory only: they are not replicated and do not survive a restart ([flo#106](https://github.com/floruntime/flo/issues/106)). Use the Flo protocol for durable key-value writes. |
| Clusters | RESP writes are served by a single-node server; a multi-member group refuses them with `-ERR`. Use the Flo protocol on a cluster. |
