...
| Flag | Description |
|------|-------------|
| `--wait <ms>` | Wait until the key exists, then return (at most 5 minutes; 0 or anything longer means 5 minutes) |
| `--block <ms>` | Wait for the **next version change**, even if the key already exists (at most 5 minutes; 0 or anything longer means 5 minutes) |
