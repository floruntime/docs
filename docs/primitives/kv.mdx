...
| Flag | Description |
|------|-------------|
| `--wait <ms>` | Wait until the key exists, then return (at most 300000 ms, 5 minutes; 0 does not wait; a longer wait is refused) |
| `--block <ms>` | Wait for the **next version change**, even if the key already exists (at most 300000 ms, 5 minutes; 0 does not wait; a longer wait is refused) |
