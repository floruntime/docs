---
title: KV
lede: Strongly consistent, versioned key-value storage with CAS, TTL, blocking gets, and namespace isolation.
---
...
```bash
flo kv get mykey
flo kv get mykey -o json   # includes version number
```

| Flag | Description |
|------|-------------|
| `--wait <ms>` | Wait until the key exists, then return; `block_ms` on the wire (at most 300000 ms, 5 minutes; 0 does not wait; a longer wait is refused) |
| `--block <ms>` | Wait for the **next version change**, even if the key already exists; `wait_ms` on the wire (at most 300000 ms, 5 minutes; 0 does not wait; a longer wait is refused) |
...
- **`--block`** is for watching — subscribe to the next change on an existing key. Even if the key exists now, it waits for a newer version. If nothing changes in time, it returns the current value with its unchanged version, so a watcher can compare and ask again; a key that is gone returns not found.
...
flo kv get config:feature-flags --block 300000
```
...
flo kv get account:42 -o json     # → value + version=11
...
flo kv get feature:dark-mode --block 300000 -n production
```
