---
title: Time-Series
lede: Columnar time-series storage with InfluxDB line protocol, structured queries, FloQL pipelines, retention policies, and downsampling.
---
...
Tag sets are **order-independent**: `host=a,env=b` and `env=b,host=a` are the
same series. Tags are stored alongside the series, so they survive a restart and
can be matched on at query time rather than only compared for exact equality.
...
# Output as JSON
flo ts read cpu --tags host=web-01 --from -1h -o json
```
...
A tag filter constrains **only the tags it names**, so a partial filter matches
any series whose tag set contains it:

```bash
# Written with two tags
flo ts write cpu --tags host=web-01,env=prod --value 72.5

# All three of these match it
flo ts read cpu --tags host=web-01 --from -1h            # partial
flo ts read cpu --tags env=prod --from -1h               # partial, other tag
flo ts read cpu --tags env=prod,host=web-01 --from -1h   # full set, any order
```

Omitting `--tags` reads across **every** tag-series for the measurement and
field, merged in timestamp order.

The CLI's `--tags` is equality-only. For negation and wildcards, use FloQL
source filters:

| Operator | Meaning | Example |
|----------|---------|---------|
| `=` | Equals | `{host=web-01}` |
| `!=` | Not equals — also matches series lacking the tag | `{env!=staging}` |
| `=~` | Glob match (`*`, `?`) | `{host=~web-*}` |
| `!~` | Glob non-match | `{path!~*-west}` |
...
Windows are aligned to the epoch (`timestamp / window * window`), so a bucket's
start does not shift with `--from` and the same point always lands in the same
bucket across queries. Each row of the result is one window.
