---
title: Streams
lede: Append-only commit log with Stream IDs, consumer groups, partitioning, tiered storage, and blocking reads.
---
...
One append — all its records and headers together — takes at most 64 KiB.
...
# The longest wait the server allows: 5 minutes (a longer one is refused;
# 0 does not wait)
flo stream read events --block 300000 --start 0-0 --limit 5
...
# Byte-based trim is NOT implemented yet — this returns an explicit error
# rather than silently trimming nothing.
...
| Blocking | `--block <ms>` | Waits up to `<ms>` for new data (at most 300000 ms, 5 minutes; 0 does not wait; a longer wait is refused) |
