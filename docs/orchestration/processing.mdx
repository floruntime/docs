...
lede: Build continuous data pipelines with declarative YAML — filter, map, aggregate, enrich, window, and route data across streams, time-series, KV, and queues without leaving Flo.
---
...
- Stream- and time-series-source pipelines resume from a **durable source cursor** rather than re-reading from the beginning. The cursor is persisted as a `processing_checkpoint` entry (debounced to ≥1s per pipeline to bound write amplification)
